  ...
  const renderStars = (rating) => {
    ...
    for (let i = 0; i < yellowStars; i++) {
      stars.push(
        <img
          key={i}
          src="src/assets/images/doctor_imgs/yellow_star.png"
          alt="Yellow Star"
        />
      );
    }
    ...
    for (let j = 0; j < greyStars; j++) {
      stars.push(
        <img
          key={j + yellowStars}
          src="src/assets/images/doctor_imgs/grey_star.png"
          alt="Grey Star"
        />
      );
    }
            ...
            <div className="doctor-list flex flex-col gap-5">
              {filteredDoctors.map((doctor, index) => (
                <div
                  onClick={handleRoute}
                  key={index}
                  className="single-doctor flex items-center content-between bg-[#CFE5FE] text-[#00305F] px-5 py-6"
                >
                  ...
                  <div className="doctor-details">
                    <p className="doctors-name text-nowrap text-[#00305F] font-semibold">
                      {doctor.name}
                    </p>
                  </div>
